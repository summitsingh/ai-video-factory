"""Focused diagnostic: get claim TEXT from reliable-topic NASA sources (Mars water +
Enceladus) to find OVERLAPPING claims that merge into corroborated (>=2 source) verified
facts. Skip Europa/Lunar (tiel-coder fails to ground them verbatim). max_attempts=2."""
...
QUERIES = [
    ("Mars+ancient+riverbed", "Mars ancient riverbed"),
    ("Mars+ancient+oceans", "Mars ancient oceans"),
    ("Mars+paleolakes+lakes", "Mars paleolakes"),
    ("Enceladus+plume", "Enceladus plume"),
    ("Enceladus+geysers+ocean", "Enceladus geysers"),
]
...
def main() -> int:
    ...
    for url, label in QUERIES:
        ...
        try:
            ...
            dt = time.monotonic() - s
            print(f"\n[OK ] {label:22s} {dt:5.1f}s -> {ok}/{len(outs)} quoted", file=sys.stderr)
            for e in outs:
                print(f"      N: {norm(e.claim)[:90]}", file=sys.stderr)
        except Exception as exc:  # noqa: BLE001
            dt = time.monotonic() - s
            print(f"\n[FAIL] {label:22s} {dt:5.1f}s -> {type(exc).__name__}: {str(exc)[:50]}", file=sys.stderr)
    return 0
